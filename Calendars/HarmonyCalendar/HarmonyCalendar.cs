...
public class HarmonyCalendar : Calendar
{
    ...
    public static DateTime[] GetSeasonalMarkers(int year)
    {
        ...
        List<SeasonalMarker> seasonalMarkers = db.SeasonalMarkers
            .Where(sm => sm.UtcDateTime.Year == year)
            .OrderBy(sm => sm.UtcDateTime)
            .ToList();
        ...
        return seasonalMarkers.Select(sm => sm.UtcDateTime).ToArray();
    }
